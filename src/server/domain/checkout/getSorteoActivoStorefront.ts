 ...
 * nombre/premio/fechas/bases + un CONTEO de participantes—, NUNCA correos ni identidades
 * (privacidad, ADR-0004). Tenant-scoped por el contexto (subdominio), jamás por input (I1). El
 * texto de las bases sale del `Tenant` (`basesSorteo`, borrador del Organizador, ADR-0008); el
 * `basesUrl` es el enlace al archivo de bases (si lo hay). Sin sorteo ACTIVO ⇒ null (no hay sección).
 */
export async function getSorteoActivoStorefront({
  ...
  const raffle = await db.raffle.findFirst({
    ...
    select: {
      ...
      fechaFin: true,
      basesUrl: true,
      // Solo el CONTEO — nunca los correos de las entries (privacidad, ADR-0004).
  ...
  return {
    ...
    totalParticipantes: raffle._count.entries,
  };
}
