...
- [ ] `POST https://sorteatelo.cl/api/webhooks/flow` con `token=E539F5A2AC2081CF4068F3F49208B4CFAEC071CO` responde 200 sin `ignorado`
- [ ] Orden `cmsovwfxe000cyg2vru7qxe7n` en estado PAGADO; Payment con `fee` registrado
- [ ] 1 RaffleEntry nueva con número 13 en el raffle `cms1zov3u000enkk28ns7ivsk` (ultimoNumero pasa de 12 a 13)
- [ ] 1 DownloadGrant para la orden, con token y expiración a 30 días
- [ ] Fila `CONFIRMACION_COMPRA` en el ledger para la orden, que termina ENVIADO (waitUntil o cron)
- [ ] Correo real entregado a la compradora (proveedorId de Resend presente; idealmente confirmación de la Organizadora)
- [ ] Re-POST del mismo token (replay) ⇒ no-op: sin ticket duplicado, sin segundo correo, número no consumido
