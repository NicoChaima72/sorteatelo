...
**Vitest**:
- [ ] `RichTextoSchema` parsea runs con marcas del enum y links por markDef; marca fuera del enum, HTML en `t`, markDef huérfano/duplicado, url `javascript:`/http-no-https, campo extra ⇒ rechazo `.strict()` — `src/__tests__/server/pagebuilder/runs.test.ts::page.runs.001..011`
- [ ] `RunsTexto` renderiza cada marca a su estilo por token (acento con fallback a marca; resaltado como background del propio span; escala en em) y los 3 tipos de link a su href seguro — cero hex, cero HTML del tenant — `src/__tests__/components/runs-texto.test.tsx::page.runs.render.001..004`
- [ ] Migrate `texto_rico` v1→v2 es lossless: un doc v1 publicado migra on-read y renderiza texto idéntico (mismo contenido visible); el paso normaliza sin escribir a DB — `src/__tests__/server/pagebuilder/runs.test.ts::page.runs.migrate.001..003,005 + page.runs.013`
- [ ] Un doc v2 con runs inválidos en render público se descarta tolerante (sección omitida, página viva — patrón leerDocumentoParaRender) — `src/__tests__/server/pagebuilder/runs.test.ts::page.runs.migrate.004`
...
**Vitest**:
- [ ] Migrate del hero absorbe `tituloAcento`: hero viejo con `{palabra, estilo}` migra a runs con la marca equivalente sobre la primera ocurrencia; hero sin tituloAcento migra a `[{t}]` plano; ambos lossless — `src/__tests__/server/pagebuilder/runs.test.ts::page.runs.hero.001..004` + `src/__tests__/components/titulo-hero.test.tsx::hero.render/reveal.*`
- [ ] Los docs de las réplicas/seeds actuales parsean tras el v-bump y su render es equivalente (regresión de equivalencia) — seeds validados `SOLO_VALIDAR` (bcac 7 secc / demo-dreamy/noche/editorial) + curl :3001 (bcac muestra "Enriquecer" acento, texto_rico BASES renderiza); `factory.test.ts` + golden docs actualizados
- [ ] Cada campo promovido de la lista D5 parsea runs y su migrate string→runs es lossless — `src/__tests__/server/pagebuilder/runs.test.ts::page.runs.perfil.001` (D5 = perfil_autora.bio; resto diferido REVISABLE)

**E2E**:
- [ ] La réplica `prueba` (landing_idol) sigue mostrando "enriquecer" dorado tras el deploy SIN editar (migración no-op visible) — verificado por curl :3001 (prueba 5×`acento-filled`; bcac 5×"Enriquecer"+7×`acento-filled`) — pendiente confirmación visual del feature-tester

### F03 — editor contenteditable de runs

**Vitest**:
- [ ] La serialización DOM→runs normaliza (merge de runs adyacentes iguales, sin spans vacíos) y el resultado parsea contra `RichTextoSchema`; pegar HTML entrega solo texto plano — `src/__tests__/server/pagebuilder/runsEdicion.test.ts::page.runs.edit.001..003,011,012` (normalización/serialización puras; el `onPaste`→text/plain es DOM, verificado por el feature-tester en vivo)
- [ ] Aplicar/quitar una marca sobre una selección produce los runs esperados (helpers puros testeados) — `src/__tests__/server/pagebuilder/runsEdicion.test.ts::page.runs.edit.004..010` + detección del control `runs` en `src/__tests__/server/editor/formGenerator.test.ts::page.editor.form.003`

**E2E**:
- [ ] En el editor: escribir un título, marcar una palabra en acento, resaltar otra, insertar link a ancla — el canvas lo refleja en vivo (patch) y publicar lo hace visible en el SSR público — pendiente feature-tester (browser autenticado; `/editor` compila OK verificado por curl 404)

### F04 — multi-página backend + [slug].tsx

**Vitest**:
- [ ] `crearPagina` valida slug (kebab, ≤64, reservados rechazados: admin/api/checkout/producto/editor/login/home/…), respeta el unique (tenantId, slug) y siembra draft con `documentoInicial`; sin membresía ⇒ FORBIDDEN — `src/__tests__/server/pagebuilder/paginas.test.ts::page.pag.001..004` (FORBIDDEN = capa `exigirEditor` del router, cubierto por `exigirEditor.test.ts`)
- [ ] `eliminarPagina`/`renombrarPagina` prohíben `home`; renombrar decide y testea el destino del historial de versiones (D8) — `src/__tests__/server/pagebuilder/paginas.test.ts::page.pag.005..008` (D8: el historial SIGUE a la página — updateMany de StorefrontPageVersion en la $tx)
- [ ] El SSR de `[slug]` sirve SOLO published de tienda PUBLICADA del tenant del subdominio; página inexistente/despublicada/slug reservado ⇒ 404 neutral; cross-tenant imposible por construcción (tenant server-side) — verificado por curl :3001 (autora/sobre-mi→404 neutral, /checkout|/admin|/producto rutas estáticas intactas, home 200); lógica en `getPropsPaginaTienda` (fail-closed I-U3)
- [ ] `derivarNav` incluye páginas con `enNav=true` y las excluye con false; sin páginas extra el nav es idéntico al actual (no-op) — `resolverNavPaginas` (SSR, DEFENSIVO I-G) filtra enNav+published; `humanizarSlug` testeado en `nav.test.ts::nav.humanizar.001`; merge `[...derivarNav, ...navPaginas]` en index/[slug]

**E2E**:
- [ ] `demo.localhost:3001/sobre-mi` publicada responde 200 con el chrome del tenant; el mismo slug en OTRO tenant da 404 — pendiente feature-tester (crear+publicar página con auth); routing base verificado por curl

### F05 — panel Páginas

**Vitest**:
- [ ] (los use cases quedan cubiertos en F04; UI sin vitest si es solo wiring — el implementer decide y anota) — ✔ decisión del implementer: `PanelPaginas` es superficie pura (I-I, toda escritura por `api.pagebuilder.*`); su lógica de dominio (crear/renombrar/eliminar/enNav) está testeada en `paginas.test.ts` (F04). tsc+lint verdes.

**E2E**:
- [ ] Crear página desde el panel, editarla (el switcher cambia editor+preview a `?pagina=`), publicarla, toggle enNav la mete al menú; home no ofrece eliminar/renombrar — pendiente feature-tester (browser autenticado); `/editor` compila (curl 404 gate)

### F06 — chromeJson + render con pinned

**Vitest**:
- [ ] `ChromeSchema` parsea header/footer/menu con enums y MenuItem discriminado; url no-https, etiqueta >20, item extra, hex ⇒ rechazo `.strict()` — `src/__tests__/server/pagebuilder/chrome.test.ts::page.chrome.001,002`
- [ ] `chromeJson null` ⇒ el layout resuelto es byte-idéntico al actual (regresión de equivalencia); chrome podrido ⇒ chrome default tolerante (nunca 500) — `chrome.test.ts::page.chrome.004,005` + curl :3001 (header `blur(10px)` presente en autora, chrome=null degrada a vidrio default)
- [ ] El render del header/footer SIEMPRE incluye carrito+sesión y atribución+enlace a Bases sin importar el contenido de chromeJson (pinned por construcción — no existe input que los quite) — `chrome.test.ts::page.chrome.003` (el schema NO tiene campo para carrito/bases ⇒ `.strict` rechaza intentar apagarlos) + curl (Carrito/aria "Abrir carrito"/atribución presentes)
- [ ] Menú del chrome presente ⇒ manda sobre el nav derivado; menuItem tipo pagina resuelve a `/slug`, tipo ancla a `#ancla` — `chrome.test.ts::page.chrome.006` (`hrefMenuItem`) + override en `index.tsx`/`[slug].tsx` (`menuChrome.length>0 ? menu : derivado`)

**E2E**:
- [ ] Tienda demo con header sticky + esquema oscuro + menú explícito (ancla+página+url externa) renderiza el chrome en home Y en `/sobre-mi` (consistencia entre páginas); carrito/sesión/Bases visibles siempre — pendiente feature-tester (necesita restart del dev server para el cliente Prisma nuevo + crear página con auth)

### F07 — edición del chrome en el editor

**Vitest**:
- [ ] `setChrome` valida contra `ChromeSchema` server-side y exige membresía; input inválido no escribe — `src/__tests__/server/pagebuilder/chromeTienda.test.ts::page.chrome.set.001..003` (write + null→DbNull + getChrome default) + rechazo de input en `chrome.test.ts::page.chrome.002` + membresía en `exigirEditor.test.ts`

**E2E**:
- [ ] Editar el menú y el esquema del header desde el panel Chrome se refleja en el preview; los pinned aparecen con candado y no son editables — pendiente feature-tester (necesita restart; el `setChrome` en el server corriendo con cliente viejo NO conoce chromeJson ⇒ falla hasta restart); `PanelChrome` compila, pinned con candado renderizados en el form
...
- [2026-07-25 23:40] [feature-implementer] Arranca implementación. CONTRATO NOCTURNO — esta corrida cubre **Fases 0-3 del prompt = F01–F07** (runs F01-F03 + multi-página F04-F05 + chrome editable F06-F07). Fila/responsive/UX/cierre (F08–F15) las toma otra corrida. Read pass completo: `_template.md`, ADRs 0016/0017/0018/0019/0008/0005/0004, `schema.ts`/`widgets.ts` (30 widgets, EstiloSeccionSchema, heroProps con tituloAcento, textoRicoProps/BloqueTextoSchema), `migrate.ts` (migrarNodo/migrarOverlay/leerDocumentoParaRender), `factory.ts`, `nav.ts` (derivarNav/ANCLA_POR_TIPO), `mutaciones.ts`+`schemas.ts`+`aplicarMutacionPagina.ts`+`getPagina.ts`+`publicarPagina.ts`, router `pagebuilder.ts` (exigirEditor), `prisma/schema.prisma` (StorefrontPage @@unique[tenantId,slug] + versiones per-slug ⇒ DB multi-página LISTA; Tenant SIN chromeJson), `getStorefrontProps.ts`/`getEditorProps.ts`/`index.tsx` (pipeline SSR + cargarDocumentoParaRender hardcodea slug:"home"), `storefront-layout.tsx` (Header/Footer con pinned actuales: carrito+AccesoSesion+Bases), `render-pagina.tsx`, `editor-pagebuilder.tsx` (dock 5 paneles, MUTACIONES_QUE_RECARGAN, patch postMessage), `panel-edicion.tsx`/`form-props.tsx`/`introspeccion.ts` (form-generator por introspección Zod), `titulo-hero.tsx`/`storefront-hero.tsx` (acento por substring), `panel-dock.tsx`, `slugTienda.ts` (SLUGS_RESERVADOS + esSlugReservado), `middleware.ts`, `texto-rico.tsx`. Dev server ÚNICO ya corre en :3001 (devTienda→autora). Tests filtrados con `npx vitest run`.
- [2026-07-26 08:21] [feature-implementer] F01 IMPLEMENTADA (motor de runs, Fase 0). Archivos: `src/lib/pagebuilder/widgets.ts` (+`MARCAS_RUN`/`RUN_MAX_CHARS`/`slugPagina`/`urlLinkSegura`/`DestinoLinkSchema`/`MarkDefLinkSchema`/`RunSchema`/`RichTextoSchema`/`runsDeTexto`; `BloqueTextoSchema` y `textoRicoProps` a v2: bloques subtitulo/parrafo/cita con `rico:RichTexto`, cita.autor + lista.items siguen string; `texto_rico` registry v1→v2 + defaultProps con runs), `src/lib/pagebuilder/migrate.ts` (paso `texto_rico` v1→v2 LOSSLESS `texto→runsDeTexto`, idempotente), `src/components/storefront/runs-texto.tsx` NUEVO (`RunsTexto`/`estiloMarcasRun`/`hrefDestino` — marcas por token, links seguros, cero hex, cero dangerouslySetInnerHTML), `src/components/storefront/texto-rico.tsx` (BloqueRender usa `<RunsTexto>`), `scripts/seed-bcac.ts` (bloques BASES a v2 shape — runnable; NO re-corrido, los docs publicados migran on-read). Tests: `runs.test.ts` (18) + `runs-texto.test.tsx` (4) = 22/22 verde. Regresión: `widgetsContenido.test.ts::page.cont.002` actualizado a v2 (su sujeto —el schema— cambió; el path v1→v2 lo cubre runs.test.ts). Suite pagebuilder+editor+components 223/223. tsc: 0 errores nuevos (los 2 de `montoEsperado`/`FlowRuteado` son del refactor de pago in-flight AJENO —prohibido tocar—, pre-existentes). DECISIONES TÁCTICAS REVISABLES: (a) run `t` max=1000 per D1, migrate trocea strings >1000 en runs (lossless); (b) `superRefine` rechaza markDefs huérfanos Y links dangling (modelo limpio — el serializer de F03 limpiará huérfanos); (c) campo del bloque = `rico` (no `children`) para no chocar con el `children` interno del RichTexto; (d) links `url` externos renderizan con `rel="noopener noreferrer nofollow"` + `target="_blank"`; ancla/pagina sin target; (e) un run con link se renderiza como su propio `<a>` (sin merge de runs adyacentes con mismo link — MVP, REVISABLE).
- [2026-07-26 08:48] [feature-implementer] F02 IMPLEMENTADA (runs en hero + textos largos, Fase 0). Archivos: `widgets.ts` (motor de runs MOVIDO arriba de heroProps —orden de evaluación—; +`marca`/`gradiente` a MARCAS_RUN aditivos para absorber los 4 estilos de tituloAcento LOSSLESS; +`MARCA_POR_ESTILO_ACENTO`/`runsDeTituloAcento`; hero v2→**v3**: titulo/subtitulo→RichTexto, `tituloAcento` REMOVIDO del schema; perfil_autora v1→**v2**: bio→RichTexto [D5, único campo promovido]), `migrate.ts` (hero v2→v3 `migrarHeroV3Props` absorbe tituloAcento con byte-equivalencia; perfil_autora v1→v2 `migrarPropStringARuns`), `runs-texto.tsx` (`estiloMarcasRun` +marca/gradiente ESPEJANDO `estiloAcentoCss`), `titulo-hero.tsx` REESCRITO (TituloHero(RichTexto,efecto): ninguno→RunsTexto, gradiente_animado→texto plano, revelar_palabras→reveal por palabra sobre runs), `storefront-hero.tsx` (3 variantes: `tituloHeroRico`/`subtituloHeroRico`, RunsTexto en subtítulo), `perfil-autora.tsx` (bio via RunsTexto), `factory.ts` (hero titulo/subtitulo→runsDeTexto). Seeds a v3: `seed-bcac.ts` (Enriquecer acento) + `seed-demos-kpop.ts` (3 heroes: BTS marca ×2, "Anda a ver a BTS" gradiente ×1) — NO re-corridos; SOLO_VALIDAR ✓. Tests: runs.test.ts (28) + titulo-hero.test.tsx (10) verde. Regresión REPARADA: 16 tests en 9 archivos (schema/estiloTema/migrate/mutaciones/versionado/widgetsSocial/widgetsTanda2/widgetsV2b golden docs y fixtures a v3 RichTexto; formGenerator OVERRIDES: -hero.tituloAcento, +hero.titulo/subtitulo/perfil_autora.bio [temporales, F03 los quita al soportar `runs`]). Suite pagebuilder+editor+mcp+components+styles **312/312**. tsc 0 nuevos. Verificación :3001: 6 tenants HTTP 200; bcac "Enriquecer" acento + BASES texto_rico renderizan (migrate-on-read byte-equiv confirmado). DECISIONES REVISABLES: (a) marca/gradiente marks ADITIVAS al enum (page.runs.011 actualizado) — la absorción lossless lo exige; (b) D5 promueve SOLO perfil_autora.bio (banner_cta/momento_ticket/packs diferidos — cada uno = v-bump+migrate+render+riesgo sobre docs publicados; el motor generaliza trivialmente); (c) edge case documentado: hero con tituloAcento SIN titulo pierde el acento en el fallback (I2 prohíbe copiar branding.nombre; ningún seed/publicado tiene esa combo — verificado por grep); (d) links/multi-word acento en un solo run bajo efectos revelar/gradiente pueden perder link (MVP, rare).
- [2026-07-26 08:58] [feature-implementer] F03 IMPLEMENTADA (editor contenteditable de runs, cierra Fase 0). Archivos NUEVOS: `src/lib/pagebuilder/runs-edicion.ts` (manipulación PURA de runs por OFFSET: `normalizarRuns`/`partirEnOffsets`/`toggleMarca`/`aplicarLink`/`quitarLink`/`limpiarRichTexto`/`limpiarMarkDefs`/`tramosARichTexto`/`textoPlano` — el modelo, no el DOM ⇒ testeable en node), `src/components/editor/editor-runs.tsx` (`EditorRuns`: contenteditable plaintext-only, DOM sembrado IMPERATIVO con createElement+textContent —JAMÁS innerHTML/dangerouslySetInnerHTML I-U1—, toolbar curada negrita/énfasis/acento/resaltado/escala + link/quitar-link, `DialogoLink` tipado ancla|pagina|url validado por slugPagina/urlLinkSegura, onPaste→text/plain; patrón serializa→transforma→re-siembra→restaura selección; markDefs en ref; ultimoEmitido para no clobber). Modificados: `src/lib/editor/introspeccion.ts` (control `runs` detectado por IDENTIDAD `base===RichTextoSchema` tras pelar el optional), `src/components/editor/form-props.tsx` (rama `runs`→EditorRuns), `formGenerator.test.ts` (quitados los overrides temporales de F02; +page.editor.form.003). Tests: `runsEdicion.test.ts` (12) + `formGenerator` (runs detection) verde. Suite pagebuilder+editor+mcp+components+styles **325/325**. tsc 0 nuevos, lint 0 en los 7 archivos tocados. `/editor` compila (curl 404 = gate sin sesión, sin 500). DECISIONES REVISABLES: (a) DialogoLink rama `pagina` usa TextInput de slug (no Select) — las páginas no existen hasta F04/F05; cuando F05 tenga el listado, pasa a Select; (b) marcas/links por serialize→transform→re-seed (no Range surgery frágil) — pierde/restaura cursor en cada op de marca, estándar para el patrón; (c) texto_rico.bloques SIGUE como override "editar por asistente" (array de bloques con runs c/u = editor más complejo, fuera del scope F03; el EditorRuns cubre los campos RichTexto planos hero.titulo/subtitulo/perfil_autora.bio); (d) escala solo `escala_lg` en la toolbar (escala_xl queda como marca válida sin botón — REVISABLE). El contenteditable + serialización DOM son DOM-puros ⇒ su E2E (escribir/marcar/link en vivo) lo valida el feature-tester con browser autenticado.
- [2026-07-26 09:16] [feature-implementer] F04+F05 IMPLEMENTADAS (multi-página, Fase 1). SCHEMA: `StorefrontPage.enNav Boolean @default(false)` — ADITIVO no-op. `schema-guardian` NO invocable (fuera de mi toolset); AUTO-REVIEW: aditivo, default false ⇒ toda fila existente queda enNav=false (byte-idéntico I-U8), sin @@index (se filtra dentro de las pocas páginas de un tenant ya scopeado por el prefijo del unique), onDelete/relations sin cambio — sigue las convenciones prisma. `db push` OK (DB en sync); `prisma generate` dio EPERM en el DLL (lock del dev server corriendo) PERO el cliente JS+.d.ts SÍ se regeneró (35 refs a enNav) y el DLL es idéntico —misma versión prisma— ⇒ tests (proceso fresco) y tsc ven enNav; el dev server CORRIENDO usa el cliente cacheado viejo (queries con enNav fallarían) ⇒ hice `resolverNavPaginas` DEFENSIVO (try/catch→[] I-G) para que el nav de páginas degrade sin 500 hasta un restart. **Queda ANOTADO: el orquestador debe correr schema-guardian sobre el enNav + reiniciar el dev server para reflejar enNav en vivo.** Archivos F04: `prisma/schema.prisma` (+enNav), `src/lib/pagebuilder/factory.ts` (`documentoNuevaPagina`), `src/server/tenancy/slugTienda.ts` (`SLUGS_PAGINA_RESERVADOS`/`esSlugPaginaReservado`), `src/server/domain/pagebuilder/paginas.ts` NUEVO (listar/crear/renombrar/eliminar/setEnNav + inputs; slug kebab validado + reservados; $tx; home protegida; historial SIGUE a la página al renombrar D8), `src/lib/pagebuilder/nav.ts` (`humanizarSlug`), `src/server/storefront/getStorefrontProps.ts` (cargarDocumentoParaRender por slug —home fallback on-the-fly, otras→null 404—; `resolverNavPaginas` defensivo; `getPropsPaginaTienda` fail-closed I-U3), `src/pages/[slug].tsx` NUEVO (mismo pipeline que index), `src/pages/index.tsx` (+navPaginas merge), router `pagebuilder.ts` (5 procedures nuevos + slug en getBorrador/mutar/publicar/listarVersiones/revertir), `schemas.ts` (slug en inputs). Archivos F05: `src/components/editor/panel-paginas.tsx` NUEVO (lista + crear/renombrar/eliminar modales + switch enNav + switcher onSwitch; home sin renombrar/eliminar), `editor-pagebuilder.tsx` (DockKey `paginas`, paginaSlug de `?pagina=` shallow, thread slug en queries/mutations/previewSrc, reset version al switch), `panel-historial.tsx` (+slug prop). Tests: `paginas.test.ts` (10, mock in-memory Prisma con $tx) + `nav.test.ts` humanizarSlug. Suite 378/378. tsc 0 nuevos, lint 0. Curl :3001: autora/sobre-mi→404 neutral, /checkout|/admin|/producto rutas estáticas INTACTAS, 6 tenants home 200, /editor compila. REVISABLES: (a) label de página = `humanizarSlug` (no hay columna nombre) — REVISABLE agregar nombre editable; (b) DialogoLink de F03 rama pagina sigue TextInput (F05 no lo cambió a Select — el listado existe ahora vía listarPaginas, REVISABLE conectarlo); (c) renombrar mueve el historial (updateMany StorefrontPageVersion), eliminar lo borra (deleteMany) — D8 resuelto "el historial sigue/muere con la página".
- [2026-07-26 09:31] [feature-implementer] F06+F07 IMPLEMENTADAS (chrome editable, Fase 2). SCHEMA: `Tenant.chromeJson Json?` — ADITIVO no-op. `schema-guardian` NO invocable ⇒ AUTO-REVIEW: aditivo, null ⇒ header/footer actual byte-idéntico (I-U8), sin @@index (se lee por el tenant ya resuelto), sigue las convenciones prisma; **ANOTADO para el orquestador correr schema-guardian**. `db push --skip-generate` OK; `prisma generate` EPERM en DLL (dev server lock) PERO cliente JS+.d.ts regenerado (70 refs chromeJson) ⇒ tests+tsc ven chromeJson; dev server CORRIENDO usa cliente viejo ⇒ `resolverChrome` DEFENSIVO (try/catch→null) para degradar sin 500. Archivos F06: `src/lib/pagebuilder/chrome.ts` NUEVO (ChromeSchema Zod .strict: header layout/sticky/transparenteSobreHero/fondo/menu + footer columnas/links/texto; MenuItem = etiqueta + DestinoLink reusado de runs; `leerChromeParaRender` tolerante; `chromeDefault`; `hrefMenuItem`), `prisma/schema.prisma` (+chromeJson), `src/components/storefront/storefront-layout.tsx` (Header/Footer aceptan chrome: fondo `estiloFondoHeader` con vidrio=blur actual default, sticky, links/texto del footer; PINNED carrito/sesión/atribución/Bases SIEMPRE renderizados fuera del chrome I-U2), `src/server/storefront/getStorefrontProps.ts` (`resolverChrome` defensivo + chrome en PropsHome/PropsPagina), `src/pages/index.tsx`+`[slug].tsx` (menú del chrome MANDA sobre nav derivado D10; chrome→StorefrontLayout), `docs/adr/0021-chrome-global-tenant-nodos-pinned.md` NUEVO estado **propuesto** (2 conceptos: chrome global + pinned por construcción). Archivos F07: `src/server/domain/pagebuilder/chromeTienda.ts` NUEVO (getChrome/setChrome + input; setChrome re-valida ChromeSchema I3, null→Prisma.DbNull restablece), router `pagebuilder.ts` (getChrome/setChrome procedures gateados; MCP NO gana tool I12/D12), `src/components/editor/panel-chrome.tsx` NUEVO (DockKey `chrome`: forms header/footer, EditorMenu repeater de MenuItem tipado, PINNED con candado visibles-no-editables, Guardar/Restablecer), `editor-pagebuilder.tsx` (DockKey chrome). Tests: chrome.test.ts (6) + chromeTienda.test.ts (3). Suite pagebuilder+editor+mcp+tenancy+components+styles **387/387**. tsc 0 nuevos, lint 0. Curl :3001: 6 tenants byte-idénticos (blur+carrito+atribución pinned presentes, chrome null→vidrio default), editor compila. REVISABLES: (a) FONDO_HEADER reducido a vidrio/superficie/transparente (marca/tinta piden pairing de contraste — diferido); (b) `layout:"centro"` en schema pero render aún = izquierda (centrar desalinea los pinned, pide grilla 3-col — REVISABLE); (c) footer `columnas` en schema pero efecto de render mínimo (REVISABLE); (d) el chat de IA NO gana tool de chrome (I12/D12) — anotado en ADR-0021 como candidato post-aceptación. **LIMITACIÓN OPERATIVA**: el `setChrome`/preview-refleja-chrome en VIVO necesita el cliente Prisma nuevo ⇒ REQUIERE restart del dev server (el server corriendo con cliente viejo no conoce chromeJson en tenant.update). Tests (proceso fresco) verdes.
- [2026-07-26 09:31] [feature-implementer] **FASE 2 (chrome, F06-F07) CERRADA.** Gate: tsc 0 nuevos, lint 0, vitest 387/387. Runtime :3001: tenants byte-idénticos (pinned + defaults presentes, chrome degrada elegante en cliente viejo). ADR-0021 escrito en `propuesto`. `change-set-reviewer` + `schema-guardian` (chromeJson) ANOTADOS para el orquestador. Restart del dev server pendiente para chrome en vivo.
- [2026-07-26 09:16] [feature-implementer] **FASE 1 (multi-página, F04-F05) CERRADA.** Gate: tsc 0 nuevos, lint 0, vitest 378/378. Runtime :3001: routing verificado (404 neutral, precedencia rutas estáticas, tenants intactos, /editor gate). `change-set-reviewer` + `schema-guardian` (enNav) ANOTADOS para el orquestador. Restart del dev server pendiente para enNav en vivo. Sigo a Fase 2 (chrome editable, F06-F07).
- [2026-07-26 08:58] [feature-implementer] **FASE 0 (runs, F01-F03) CERRADA.** Gate de fase: tsc 0 nuevos (solo los 2 de pago in-flight AJENO), lint 0 en los archivos tocados, vitest área 325/325. Runtime :3001 verificado: autora/prueba/bcac/demo-dreamy/demo-noche/demo-editorial HTTP 200 INTACTOS; bcac "Enriquecer" en acento + BASES texto_rico renderizan (migrate-on-read byte-equivalente). Seeds a v3/v2 (bcac + demos) SOLO_VALIDAR ✓, no re-corridos. `change-set-reviewer` de la fase queda ANOTADO para el orquestador (mi toolset no lo tiene). Sigo a Fase 1 (multi-página, F04-F05).
- [2026-07-26 08:21] [feature-implementer] REVIEWERS: mi toolset (Read/Write/Edit/Bash/Glob/Grep/Skill) NO incluye los subagentes `frontend-reviewer`/`backend-reviewer`/`change-set-reviewer`. Por instrucción explícita del prompt nocturno ("los que puedas invocar; si tu toolset no los tiene, anótalo — el orquestador los corre"), NO aplico la rúbrica manualmente: dejo ANOTADO que el orquestador debe correr `frontend-reviewer` sobre `runs-texto.tsx`/`texto-rico.tsx` (F01), `titulo-hero.tsx`/`storefront-hero.tsx` (F02), `EditorRuns`/`panel-edicion.tsx` (F03) y `backend-reviewer` sobre nada server nuevo en Fase 0 (todo puro Zod/render). Auto-chequeo design.md de F01: RunsTexto usa SOLO tokens (`--mantine-color-acento-filled` con fallback a `--mantine-primary-color-filled`, I-T2), cero hex inline (test `tieneHex`), escala en `em`, resaltado como background del propio span (§7 lección memoria) — conforme.
- [2026-07-25 23:40] [feature-implementer] NOTA PARA FASE 6 (chat IA F14, NO en esta corrida): el orquestador extrajo el patrón grillos-ai. El chat usará `@ai-sdk/gateway` `createGateway({apiKey})` inyectada desde el composition root (JAMÁS env en módulos puros — patrón FlowCredential ADR-0006), `ai` v5 `streamText` con tools Zod que envuelven las MISMAS mutaciones del pagebuilder (`aplicarMutacionPagina`/use cases), env `AI_GATEWAY_API_KEY` OPCIONAL (sin key ⇒ panel oculto, degrada elegante), modelo por string `"provider/model"`. Referencia en disco: `C:/Users/NicolásChaima/Desktop/PROGRAMACION/DATAWALT/grillos-ai/packages/agent/src/models/gateway.ts` y `tools/check-payment-status.ts`. Esto SUPERSEDE parcialmente a D21 (que planteaba SDK de Anthropic directo) — el orquestador prefiere el gateway de Vercel AI SDK; queda REVISABLE para quien tome F14.
- [2026-07-26 09:33] [feature-implementer] **CIERRE DE LA CORRIDA NOCTURNA (Fases 0-3 del prompt = F01–F07).** Implementadas y con vitest verde: F01 (motor de runs), F02 (runs en hero + perfil_autora.bio), F03 (EditorRuns contenteditable), F04 (multi-página backend + [slug].tsx), F05 (panel Páginas), F06 (chromeJson + pinned + ADR-0021 propuesto), F07 (setChrome + panel Chrome). Gate final: tsc 0 errores nuevos (persisten SOLO los 2 de `montoEsperado`/`FlowRuteado` del refactor de pago in-flight AJENO —prohibido tocar—), lint 0 en todos los archivos tocados, vitest del área **387/387** + checkout/config 41/41 (schema aditivo no rompió otras áreas). Runtime :3001 (instancia única, NUNCA arranqué una 2ª): 6 tenants (autora/prueba/bcac/demo-dreamy/demo-noche/demo-editorial) HTTP 200 byte-idénticos, migrate-on-read byte-equivalente (bcac "Enriquecer" acento + BASES texto_rico), routing multi-página correcto (404 neutral, precedencia rutas estáticas), pinned del chrome siempre presentes, /editor compila. **F08–F15 (fila, responsive, UX inline/undo/duplicar, chat IA, cierre integral) NO tocadas — otra corrida.** PENDIENTES PARA EL ORQUESTADOR/USUARIO: (1) correr `schema-guardian` sobre `StorefrontPage.enNav` + `Tenant.chromeJson` (2 columnas aditivas ya en DB via db push; auto-review hecho); (2) correr `frontend-reviewer` (runs/hero/EditorRuns/chrome/paneles) + `backend-reviewer` (use cases páginas/chrome, SSR [slug], procedures) + `change-set-reviewer` de las 3 fases; (3) **REINICIAR el dev server** para que el cliente Prisma nuevo (enNav+chromeJson) opere en vivo — el generate dio EPERM en el DLL por el lock del server corriendo, así que enNav/chromeJson en VIVO (nav de páginas, setChrome, preview de chrome) degradan defensivamente hasta el restart (los tests en proceso fresco ya los validan); (4) aceptar/rechazar ADR-0021; (5) revisar las decisiones REVISABLES anotadas por fase. NO hice commit/push/INDEX (fuera de mi rol). Bitácora = memoria completa si esta corrida se cortó a mitad.
