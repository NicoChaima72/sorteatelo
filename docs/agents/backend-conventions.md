...
| `panelProcedure` | **El borde del panel de Organizadores/Operador** (F05, ADR-0005/0022). Exige sesión y carga SERVER-SIDE `ctx.acceso: AccesoPanel` = `{ userId, email, esOperador, tenantIds, tenantIdDelHost }` — `esOperador` de `PLATFORM_OPERATOR_EMAILS`; `tenantIds` de los `TenantMembership` del usuario, en el **orden canónico** (`createdAt asc, id asc`, único en todo el sistema) vía `cargarMembresiasCanonicas` (`src/server/panel/membresias.ts`); y `tenantIdDelHost` = la Tienda del **subdominio** donde corre el panel (ADR-0022), resuelta del host con `resolverTenantAdminDesdeHost` — que a diferencia de la del storefront **no exige tienda publicada** (una tienda en CONFIGURACION o SUSPENDIDA se sigue administrando). **No gatea por membresía**: un usuario sin membresía y sin rol Operador pasa el procedure, pero cualquier use case resuelve su Tienda con **`resolverTenantDelPanel(ctx.acceso)`** y tira `FORBIDDEN` (fail-closed en la capa de datos) — así `getAccesoActual` decide qué renderizar sin tirar el request. Ese helper alimenta el host a `resolverTenantAutorizado` como `tenantIdSolicitado` **y le declara `esOperador: false`** (D11: en esta superficie el rol no autoriza nada; la política pura no cambió y conserva su rama de Operador para `operadorProcedure` — el input SELECCIONA entre lo autorizado, jamás autoriza) y es **fail-closed sin host**: murió el fallback "la primera membresía", que permitía ver una tienda y operar otra. El host es server-authored (middleware + re-parseo en el contexto) ⇒ el `tenantId` con el que se scopea TODA query del panel **jamás sale del input** (I1; H1 de datawalt-app). |
