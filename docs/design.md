...
Cuatro familias, cargadas con `next/font/google` en `src/config/fonts.ts` y cableadas por CSS var desde `_app.tsx` (el colector de `next/font` del pages router solo recorre `_app`/páginas, NO `_document`). Reemplazan a Manrope/Sora de la ruta violeta.

- **Fraunces** (`--font-heading`, variable): **headings** del theme (`theme.headings.fontFamily`) — serif cálida, aire editorial/premium que contrasta con el mono de los números (decisión 2026-07-24, elegida en `/prototipo/headings` sobre 5 candidatas; SUPERSEDE a Bricolage como fuente de headings). No es de Grillos (Recoleta/Refinder son licencias pagas ajenas).
- **Bricolage Grotesque** (`--font-display`, variable, peso **800**): **el wordmark de marca** (lo consume directo el componente `Wordmark`). El carácter «impreso popular» del talonario. Ya NO es la fuente de los headings.
...
- Jerarquía por **familia + peso + tamaño**: headings en Fraunces, wordmark en Bricolage 800, texto en Instrument, números en Plex Mono.
...
- **El chrome del panel SÍ se rediseña — gramática «Oscuro + calmo»** (prototipo aprobado por el usuario tras 3 rondas, 2026-07-18; SUPERSEDE el bullet previo "el chrome del panel NO se rediseña"). Las reglas del panel/admin:
  - **Rail lateral tinta COLAPSABLE.** Navegación en un `<aside>` con fondo **tinta** (`var(--mantine-color-black)` = `#191b22`, ya token) que **colapsa entre icon-only (~76px) e icon+label (~256px)**, con el estado **persistido** (localStorage) entre navegaciones. El wordmark va **invertido** (lockup negativo — pincelazo blanco) al expandir y como isotipo solo al colapsar. NavLink activo/hover re-estilados para fondo oscuro (activo = pill cobalto `filled`; inactivo = texto `gray-4` sobre tinta; hover = `dark-6`). El mecanismo es `AppShell` de Mantine con **ancho de navbar dinámico** (D3): el `collapsed.mobile` sigue dando el Burger + Drawer gratis; el fallback documentado es un layout flex propio.
  - **Cards SIN borde, elevación por SOMBRA difusa.** Las superficies del panel usan la primitiva compartida **`PanelCard`** (`src/components/admin/panel-card.tsx`): `Card` con `withBorder={false}`, `radius="lg"` y una **sombra difusa** tomada de un **token único** — la constante `SOMBRA_PANEL` = `0 1px 2px rgba(25,27,34,.04), 0 6px 20px rgba(25,27,34,.06)` (la tinta `#191b22` = `rgb(25,27,34)` con alpha). Un solo lugar para ajustar la elevación del panel (cero sombra hex dispersa, §9). NO se overridea el `Card` global del theme (rompería storefront/landing, que tienen su gramática suave propia).
  - **Contenido full-width.** El canvas del panel ya **no** cap­ea a `max-w-6xl`: el contenido ocupa el ancho completo bajo el gutter responsive del `AppShell`.
  - **Canvas hundido / gray-0.** El fondo del área de contenido es un **gris muy claro** vía `light-dark(var(--mantine-color-gray-0), var(--mantine-color-dark-8))` — más neutro y calmo que el celeste `hundido` (que se mantiene en las superficies de marca: landing/login). Las `PanelCard` van en blanco (body) sobre ese gris.
  - **Topbar sin borde inferior.** El `AppShell.Header` va **sin línea divisoria** (`AppShell withBorder={false}`), sobre el body, liviano: chip de tienda (con `ColorSwatch` de su color — el único color-desde-dato del admin), búsqueda ⌘K, "Ver mi tienda", menú de cuenta.
  - Radio del panel: `lg` en las cards (más suave que el `md` global). La **gramática suave de las superficies de MARCA (landing/login) se mantiene intacta** (radios 12/18, sombra difusa de color, chip de ticket con muescas): son familias distintas y no se tocan en este rediseño.
...
- Superficies clave: **landing pública de plataforma** en el apex (bandas del talonario), **login split cobalto**, storefront del Comprador (catálogo/carrito/checkout mobile-first, tematizado per-tenant — plantilla rica), **panel de Organizador** (chrome «Oscuro + calmo», §4: `AppShell` con **rail tinta colapsable** + topbar sin borde + canvas gray-0 full-width + `PanelCard` sin borde con sombra difusa — Resumen/productos/ventas/sorteo/configuración), panel del Operador. El Resumen usa la gramática rica de dashboard: saludo + badge de estado, KPIs con número **mono** + delta + **sparkline** donde hay serie, gráfico de ventas de 14 días y card "Tu sorteo" (countdown + conteo de tickets).
...
- La data-viz del panel usa **`@mantine/charts`** (`@mantine/charts@7.17.8`, empareja con `@mantine/core@7.17.8`; corre sobre `recharts@^2.13.3` — su peer-dep, resuelto bajando el `recharts@3` que era dep fantasma no importada). Su CSS (`@mantine/charts/styles.css`) se importa en `_app.tsx` DESPUÉS de `@mantine/core/styles.css`. Hereda los colores del theme por token.
- **Uso en el Resumen del panel**: los KPIs con serie diaria (ventas pagadas, ingresos) muestran un **`Sparkline`** (color por token, `sorteatelo.6`); el **gráfico de ventas de 14 días** es un **`BarChart`** de una sola serie, con la barra de **"hoy" en `amarillo`** y el resto en **cobalto** (`sorteatelo`). Restricción dura (se mantiene): **nunca más de 5 series**; grid lines sutiles; tooltips discretos; leyenda oculta cuando la serie es única. Paleta de charts derivada de la paleta de marca (§2 — cobalto/teal/amarillo/ámbar como series, en ese orden de prioridad), **cero hex inline** (color por prop de token, ej. `"sorteatelo.6"` / `"amarillo.6"`).
...
- **Entradas de la landing** (dependencia `motion`, ex framer-motion): las secciones de la landing pública entran con **fade + translate leve** al hacer scroll (una sola definición reutilizada, `RevelarAlScroll`).
- **Motion en el storefront** (ADR-0020, catálogo v2 F03): `motion` se permite en el storefront **SOLO vía la primitiva compartida `src/components/storefront/animar.tsx`** (`LazyMotion + domAnimation`, `Animar`/`AnimarItem`/`useCountUp`) — ningún otro archivo de `src/components/storefront/` lo importa (test estático). Reglas duras: **SSR-visible** (el HTML público jamás sale `opacity:0`; se anima solo client-side y solo bajo el fold — endurece `RevelarAlScroll`), **CLS=0** (solo `transform`/`opacity`/`filter:blur`), presets = enum cerrado (duración/ease FIJOS, nunca desde el documento). Microinteracciones (hover-lift, marquee, pulso CTA/segundos) = **CSS puro** en `globals.css`, gateadas por `prefers-reduced-motion`. Fuera de `animar.tsx` y de la landing, `motion` sigue prohibido (panel incluido).
- **Colapso del rail del panel** (§4, chrome «Oscuro + calmo»): la transición de ancho del rail (76px ↔ 256px) es **CSS puro** — una transición de `width` calmada (~200ms, ease) sobre el elemento del navbar (más el offset del contenido vía `AppShell.transitionDuration`), **no** la librería `motion` (que sigue prohibida en el panel). Como la transición vive en props/estilos inline del `AppShell` (no en clases Tailwind, por eso `motion-safe:` no aplica ahí), el respeto a `prefers-reduced-motion` se gatea por JS con `useMediaQuery("(prefers-reduced-motion: reduce)")`: con reduced-motion la transición es `none`/`0` y el ancho cambia al instante, sin animar.
- **Talonario vivo**: el sello «¡SALE!» rota entre los números vendidos cada ~3s; se detiene por completo con `prefers-reduced-motion`.
- Respetar `prefers-reduced-motion` **globalmente y siempre** (Mantine lo respeta con `theme.respectReducedMotion: true`; el talonario vivo, `motion` y las microinteracciones CSS lo respetan explícitamente): con reduced-motion las secciones aparecen **completas, sin animar** (nunca ocultas), y el count-up muestra el valor final.
...
- **Excepción — layout de correo** (`src/server/domain/correo/layoutCorreo.ts`; **corregida 2026-07-26**, feedback del usuario sobre los correos reales): un correo no puede leer `var(--mantine-color-*)` (el buzón del Comprador no tiene el theme), así que el color viaja como hex — pero con DOS orígenes y dos reglas distintas. (1) **El cuerpo del correo se tematiza con la marca de la TIENDA, no con El Talonario**: color-desde-dato del `Tenant` (`colorPrimario`, tonos derivados determinísticamente — mismo seam que el storefront/`generarEscalaColor`; sin color o hex inválido ⇒ tonos del theme base, misma degradación que `overrideDesdeBranding`) y el logo del tenant con fallback nombre-como-texto (§5.2). Es la MISMA excepción de color-de-dato que ya rige en el storefront: dato del tenant, no token de marca. (2) **El chrome de plataforma que sobrevive** («vía Sortéatelo» + pie con wordmark y disclaimer) usa hex de plataforma que son **copia literal de tonos que existen en `theme.ts`**, cada uno anotado con su token — copiar sí, inventar un tono nuevo no. Los roles de §3 se traducen a stacks de sistema (serif = rol de Fraunces, sans = texto, mono = números — los Números del sorteo se renderizan como *tickets* en mono), que son los fallbacks que el theme ya declara; todo `font-family` termina en familia genérica. Dentro del cuerpo los tonos del tenant se usan por **rol** y no a ojo (banda del encabezado, franja/plumón, perforación, acento de enlaces y Números), el **contraste se calcula** —titular en blanco o tinta según la luminancia de la banda; el acento baja por la escala hasta ≥4,5:1 contra el fondo del boleto— y el logo va sobre un **chip blanco** cuando se apoya en la banda de color (§5.2 cubre su AUSENCIA, no su legibilidad).
...
- **Dark mode**: el **panel/admin NO tiene modo oscuro** (decisión del usuario, 2026-07-19 — SUPERSEDE el toggle previo). El chrome del admin se **fuerza a `light`** envolviendo el subtree en `data-mantine-color-scheme="light"` (sin tocar `_app.tsx`, para no romper el dark per-tenant del storefront); el rail vuelve a `dark` localmente solo para su superficie tinta. El canvas del panel es `gray-0` fijo (no `light-dark()`). El **storefront** SÍ conserva su modo claro/oscuro **per-tenant** (dato de `TemaPagina`, no toggle de usuario).
- En la landing, **dos secciones blancas nunca adyacentes** (regla registrada del usuario): la secuencia de bandas alterna color. Secuencia vigente — **9 bandas** tras el reposicionamiento sorteo-first (`tasks/26-07-25-landing-reposicionamiento.md` D8, aprobada por el usuario): **AZUL** (hero) → **BLANCA** (cómo funciona) → **AMARILLA** (momento clave) → **BLANCA** (hazlo tú mismo) → **AMARILLA** (precio) → **BLANCA** (confianza) → **GRIS** (FAQ) → **AZUL** (boleto CTA) → **TINTA** (footer). Las dos secciones nuevas entraron **intercaladas** justamente para no juntar blancas, y ninguna sección existente se recoloreó: el amarillo lotería es el color del momento que importa, y "cuánto cuesta" es uno de esos momentos. Agregar o mover una sección obliga a re-verificar la alternancia y a actualizar esta línea.
...
- [x] **OG raster**: resuelto en `tasks/26-07-25-landing-reposicionamiento.md` F05. `public/og.svg` sigue siendo la FUENTE editable; `public/og.png` (1200×630) es el rasterizado que consume `og:image`, con **URL absoluta** (`https://sorteatelo.cl/og.png` vía `src/components/landing/seo.ts` — una ruta relativa tampoco la resuelven los crawlers). El raster se produce con **Chrome headless** (`--headless --screenshot --window-size=1200,630`) sobre un HTML que embebe el SVG y las woff2 de marca en base64 desde `videos/hyperframes/_brand/fonts/` — sin esa embebida el PNG sale en la serif del sistema, y el repo no tiene sharp/resvg. **Si se edita `og.svg` hay que regenerar el PNG a mano**: no hay gate automático que los mantenga en sync (un test solo verifica que el PNG exista y mida 1200×630). Al rasterizarlo por primera vez con las fuentes reales apareció un defecto que el SVG arrastraba: el plumón cruzaba «Tu sorteo.» como un TACHADO — la geometría del trazo se corrigió a subrayado bajo la línea de base.
