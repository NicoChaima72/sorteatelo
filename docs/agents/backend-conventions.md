...
Todo trabajo diferido del proyecto —enviar correos programados, reconciliar estados con un proveedor— corre como **cron de Vercel declarado en `vercel.json`**, nunca como cola externa ni como agendamiento delegado en un tercero (ADR-0027). El cron de Vercel es **best-effort**: puede saltarse una corrida y puede duplicarla. Por eso el job se escribe con esta forma y no otra:

- **El job pregunta «¿qué está vencido y sin hacer?», jamás «¿qué toca justo ahora?»**. La segunda pregunta pierde trabajo cuando la corrida se salta y lo repite cuando se duplica. La primera se autocorrige: una corrida perdida se recupera en la siguiente, una duplicada no encuentra nada que hacer.
- **El estado vive en una tabla-ledger, no en la expresión cron**. La fila se escribe cuando el trabajo queda pendiente y es la fuente de verdad de qué falta; el schedule solo decide cada cuánto se mira. Corolario: no hay columna `programadoPara` — agregarla convertiría el job en disparo puntual y traería de vuelta el problema.
- **Idempotencia por constraint de la DB, no por lógica**: un `@@unique` sobre la clave natural del trabajo + `createMany({ skipDuplicates: true })` para encolar. El que decide es Postgres; así dos corridas concurrentes no necesitan lock distribuido.
- **Claim por compare-and-swap antes del efecto**: `updateMany({ where: { id, estado: PENDIENTE, intentos: n }, data: { intentos: n + 1 } })`. Si el `count` vuelve 0, otra corrida ganó la fila y esta la suelta. Todo camino que ejecute el efecto —incluido un `waitUntil` fuera del cron— pasa por el mismo claim.
- **El adapter del proveedor garantiza la ALINEACIÓN del lote, o falla**: cuando el job manda N trabajos en batch y mapea la respuesta por posición (`ids[i] → filas[i]`), una respuesta `2xx` con menos ids de los pedidos NO se acepta compactando el array — se lanza. Un hueco en el medio corre todas las posiciones siguientes: el job le escribiría a una fila el identificador de otra y dejaría como pendientes trabajos ya ejecutados, que en la corrida siguiente se repiten. El caller, además, no confía: si igual recibe menos ids que trabajos, marca el resto como anomalía con error visible en vez de dejarlos pendientes en silencio.
- **La clave de idempotencia del proveedor se deriva del CONTENIDO del lote** (hash de los ids de las filas), nunca de la hora ni de un contador. Es la segunda línea de defensa —tapa la ventana entre "el proveedor aceptó" y "alcancé a confirmar"— y solo sirve si el reintento del mismo conjunto reusa la clave.
- **Modo de falla explícito y elegido**: registrar-primero-y-después-actuar deja, si el proceso muere en el medio, trabajo sin hacer y recuperable. El orden inverso deja trabajo hecho dos veces, que suele ser irrecuperable. Para efectos visibles al Comprador (correos, cobros) se elige siempre el primero.
...
- **Un CLI que OTORGA algo irreversible corre en seco por default.** La norma de la casa es aplicar directo (`backfill:product-files`, `flow:planes`), y está bien mientras el efecto sea *aditivo y recuperable*: un backfill crea filas espejo de un dato que ya existía, y el bootstrap de planes es idempotente contra un proveedor. Cuando en cambio la corrida **concede un derecho comercial** que ningún proceso posterior revoca —el grandfathering de F07 deja tiendas exentas **a perpetuidad**—, el default se invierte: **listar las candidatas y exigir un `-- --aplicar` explícito** para escribir. El operador tiene que poder ver a quiénes antes de confirmar, y un flag mal escrito tiene que degradar a listar (nunca a aplicar). El núcleo recibe el `aplicar: boolean` y devuelve el mismo resultado estructurado con un estado distinto por fila (`a-crear` vs `creada`), así el modo seco se testea igual que el otro. Ej.: `npm run grandfather:tiendas` (`src/server/facturacion/grandfatheringTiendas.ts` + `scripts/grandfather-tiendas-publicadas.ts`).
- **La idempotencia de un script la garantiza la DB, no una heurística del script.** El `where` que filtra las candidatas es la primera línea de defensa, pero el árbitro es un `@unique` (o el `skipDuplicates` de un `createMany`): contra una DB VIVA, entre el `findMany` y el `create` puede pasar cualquier cosa. Los dos errores de carrera se leen como RESULTADOS y no abortan el barrido — `P2002` (otro proceso creó justo la fila que íbamos a crear ⇒ es el resultado deseado) y `P2003` (la fila padre se borró ⇒ ya no hay nada que hacer) —, porque abortar deja a medias justo la corrida que tiene que poder repetirse. Precedentes: `backfill-product-files.ts` y `grandfatheringTiendas.ts`.
